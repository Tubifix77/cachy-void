...
install_networkmanager() {
    ...
    # DNS handover (found the hard way on the Medion): NM's default rc-manager is
    # 'symlink', which REFUSES to touch a pre-existing regular /etc/resolv.conf —
    # and dhcpcd leaves exactly such a file behind. Result: the machine stays on
    # dhcpcd's last nameserver forever (fine at home, dead at the next hotel — the
    # exact roaming case this flag exists for). Two-part fix: force rc-manager=file
    # so NM (re)writes resolv.conf on every connection change, and retire a
    # dhcpcd-generated resolv.conf so NM takes over immediately.
    local nmtmp; nmtmp="$(mktemp)"
    cat > "$nmtmp" <<'NMCONF'
[main]
# cachy-void: always (re)write /etc/resolv.conf on connection changes.
# NM's default 'symlink' mode refuses to touch a pre-existing regular file
# (e.g. one left behind by dhcpcd), stranding the machine on a stale nameserver.
rc-manager=file
NMCONF
    install_file "$nmtmp" /etc/NetworkManager/conf.d/10-cachy-dns.conf 0644 root root
    rm -f -- "$nmtmp"
    if [ -f "$(rp /etc/resolv.conf)" ] && grep -q "Generated by dhcpcd" "$(rp /etc/resolv.conf)"; then
        if $DRY_RUN; then log "[dry-run] retire dhcpcd-generated /etc/resolv.conf (NM regenerates it)"
        else
            cp -a "$(rp /etc/resolv.conf)" "$(rp /etc/resolv.conf.pre-cachy.$TS.bak)"
            rm -f "$(rp /etc/resolv.conf)"
            warn "retired dhcpcd's /etc/resolv.conf (backed up) — NetworkManager regenerates it"
        fi
    fi
