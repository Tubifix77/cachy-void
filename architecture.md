...
- Void's `kernel.d` post-install hooks handle dracut initramfs and GRUB entries. **Verified (2026-08-18):** Void's `grub` package ships both `/etc/kernel.d/post-install/50-grub` and `post-remove/50-grub`, so on a GRUB-owning Void host `grub.cfg` is regenerated by Void itself on every kernel install *and* removal. The updater therefore must **never** run `grub-mkconfig`/`update-grub` — it would duplicate that hook, and `mkconfig` is deliberately absent from the §4.1 grant (only `grub-set-default`/`grub-reboot`/`grub-editenv`). Regenerating the config once at provisioning is `deploy.sh --with-grub`'s job; on a foreign-bootloader host (§8.6 `external`) nothing of ours touches it at all. The DKMS side is the same mechanism: `/etc/kernel.d/post-install/10-dkms` rebuilds out-of-tree modules per installed kernel (live evidence: nvidia470 built for stock 6.12.95_1 plus both self-built cachy kernels).
- Old kernel cleanup is **manual only**: `vkpurge list` / `vkpurge rm <ver>`. Always keep the previous known-good kernel installed until the new one has survived a real gaming session. The updater never purges kernels (§4.7) — a kernel that boots healthy today can still fail later on a path not yet exercised (suspend, a codec, a device), and a rebuild costs hours, so the destructive half stays human. What the updater *does* owe the user is visibility, so `--status` tier [4] and `--clean` **annotate** the inventory instead of dumping version strings: per-entry size, and a computed role — `[rollback target: KEEP]` (matches `known_good.kver`), `[currently running: KEEP]` (matches `uname -r`), or a spare carrying its exact `sudo vkpurge rm <ver>` command. Two safety properties: `vkpurge list`'s `current` entry is the `/boot/vmlinuz-current` **symlink** and is filtered out (never offered for removal), and when more than one spare accumulates a **warning** names the count and total size ("one spare is enough") — the owner's call: don't auto-purge, but don't let them pile up silently either.
...
**The build space is relocatable (`[build] masterdir`).** A kernel build needs ~20 GB of transient tree and this floor asks for 30 GB free — on a 62 GB laptop root that is a permanent third of the disk reserved for something run monthly, and it is a large enough tax that the rational response is to keep the userspace overlay and abandon the BORE half. So the chroot may live on another disk: the setting is passed to xbps-src as `-m`, never written into `void-packages/etc/conf` (which `deploy.sh` regenerates and would silently clobber). `Config.build_space` is the single question every consumer asks — the preflight, the disk report, `locate_dotconfig`'s G2 glob and the xbps-src invocation must never disagree about where the build is.

**Only the masterdir moves; the hostdir never does.** The hostdir holds `binpkgs`, which is the local repository named by absolute path in `/etc/xbps.d/00-cachy-overlay.conf`. On removable media an unplug silently removes the overlay repo from xbps' view. The masterdir is transient and rebuilt on demand, so losing it costs a `binary-bootstrap`, not a system.

**Choosing one is validated, and the refusals are specific** (`--build-space PATH`, or the GUI's *Kernel build space…*): it must be a writable directory, with `min_free_gib` free, not mounted `noexec` (a chroot executes what it builds), and not on a filesystem that cannot host a root tree — vfat/exfat/ntfs/fuseblk have no unix ownership, permissions or symlinks, and a build there fails deep inside a package rather than at the point the directory was chosen. USB-attached disks are allowed but always *named*, because unplugging one mid-build ends the build. That last check reads the device's **sysfs path** for `/usb`, not `/sys/block/<dev>/removable`: an enclosure holding an ordinary SSD reports `removable = 0` (the drive is fixed; the bridge is what unplugs), which is exactly the disk a user would choose, so the flag would have fired for card readers and nothing else.

**Setting it never escalates.** `updater.toml` lives in `/etc` and is root-owned, which is correct — the build space is a property of the machine, not of whoever is logged in. When the file cannot be written, the command prints the exact edit and stops. A tool that quietly acquired root to rewrite its own config (the file that decides what gets built) would be a worse tool than one that asks.
...
- `external`: **something other than our GRUB owns boot** — `/boot/grub/grub.cfg` is absent on a real (non-WSL) machine. Two quite different shapes land here and both are handled identically: another distro's GRUB chain-booting Void (multi-boot), typically through an operator-maintained evergreen symlink such as `/boot/vmlinuz-current` — **note that this symlink is a convention, not a Void mechanism**: none of the `/etc/kernel.d` hooks Void ships creates or repoints one (they belong to bootloaders — grub, systemd-boot, efibootmgr, lilo, u-boot, m1n1 — or to dkms), so on this class the updater reports what it can observe rather than assuming any particular arrangement (§8.6b), *and* a single-OS Void deliberately using **systemd-boot, rEFInd, EFISTUB or Syslinux** — `void-installer` offers "none" for the bootloader step and those are all supported alternatives. For the second shape the word "foreign" in the reason string is a misnomer worth knowing about: the bootloader is the owner's own, it simply is not GRUB, so there is no `grubenv` to stage a one-shot into. The treatment is right either way (bookkeeping only, manual fallback); implementing per-bootloader staging for systemd-boot et al is explicitly not in scope. Kernels boot fine here; only menu control is impossible. Staging is **bookkeeping-only**: record the candidate, `staged_boot_id`, and the services snapshot (state → STAGED) and issue **zero** bootloader commands. The §8.7 confirm battery and promotion run identically — promotion advances `ported_version`/`known_good` without any `grub-set-default` — and both fallback (unhealthy candidate) and watchdog response are **manual**: the operator selects the known-good entry in the foreign menu. The watchdog therefore never fires an *active* rollback in this mode; a trip records CANDIDATE_UNHEALTHY and instructs the operator. *(Added after the first real-hardware kernel run: a Debian-owned-GRUB host was lumped into `skip`, so a healthy candidate boot was never promoted.)*
...
### 8.6b Boot-path verification (read-only)

Between "kernel built and installed" and "kernel the firmware can actually load" sits a step nothing used to check. This section closes it **without taking any new privilege and without ever writing to a bootloader** — the updater confirms an *outcome*, it does not produce one.

Who owns the step, per class:

- **We own a GRUB menu** (`oneshot` / `manual` / `manual-unsafe`): regenerating `grub.cfg` is **Void's** job, not ours. The `grub` package installs `/etc/kernel.d/post-install/50-grub`, which runs `grub-mkconfig -o /boot/grub/grub.cfg` on every kernel install — the same hook mechanism whose sibling `10-dkms` is proven to fire for `linux-cachy`. The updater therefore **never** regenerates the menu (`grub-mkconfig` is deliberately absent from the §4.1 grant; see `rejected-ideas.md`). What it does is parse the resulting `grub.cfg` for an entry matching the new kernel release and report:
  - entry found → confirmed bootable, with the resolved ref(s);
  - **no** entry → **warn**, and name the likely cause: the `50-grub` hook did not run or failed (a `/boot` that was not mounted at install time does exactly this). Remedy offered to the operator, never executed for them: `sudo xbps-reconfigure -f linux-cachy`.
- **`external`** (foreign bootloader): there is no menu of ours to inspect, and no distro mechanism to lean on either. The updater scans `/boot` for a **symlink whose target names the new kernel release** — matched by version, so any operator naming convention is found rather than one hard-coded name — and reports which of two situations holds: a symlink now points at the new kernel (the usual multi-boot arrangement: the other OS's entry loads the symlink, so the kernel boots with that bootloader untouched), or none does, in which case the kernel exists only under its versioned name and **an entry naming a versioned path still boots the old kernel**. The second case is the one worth catching before a reboot rather than after.
- **`skip`** (WSL/containers): nothing to verify, stated as such.

One distinction is normative because conflating it is how a safety feature becomes noise: **"the entry is not there" and "we could not look" are different outcomes.** An unreadable `grub.cfg` (permissions, a `/boot` that is not mounted) yields *unknown* and never a warning; only a readable menu that genuinely lacks the entry warns. Reporting is emitted at staging time and reflected in the `--status` kernel tier.

**Staged-candidate visibility (same principle, different gap).** A kernel that is built, installed and waiting for its trial boot is a real state the user is living in, and it appeared nowhere: `--status` tier [3] showed the BORE pin, the known-good kernel and upstream drift, but never "a candidate is staged". It now states the candidate and — critically — **the fallback that actually applies on this host**, taken from the recorded staging mode rather than described generically: under `oneshot` the next power cycle returns to the known-good kernel unattended; under `external` the operator picks it in the foreign menu; under `manual` the default stays on known-good and the candidate must be selected. Promising an automatic return on a host that cannot perform one would be worse than silence. The `CONFIRMING`, `CANDIDATE_UNHEALTHY` and `ROLLED_BACK` states are surfaced in the same tier, the last two stating plainly that only the kernel path is frozen while userspace updates continue (§8 preamble).
...
**Leaving a frozen state: `--kernel-ack`.** §8.8's last row has always required a human acknowledgement to return to `TRACKING`, and for a long time the command it names did not exist — so a box that landed in a frozen state was stuck short of editing `kernel-state.json` by hand. `cachy-void-update --kernel-ack` closes that: it reports what is frozen (and, when there is no candidate, says plainly that the freeze is probably a mislogged health blip), states that userspace updates were never affected, asks for confirmation, archives any candidate into `history` so the episode is not erased, and clears the freeze. It installs, removes, re-stages and writes **nothing** else. `--status` and `--pending` surface a freeze **with or without a candidate** — requiring one is how a frozen kernel path stayed invisible on the box that produced the finding.

**Manual recovery must be reachable without a terminal.** The automatic paths above cover a candidate that fails *during* its trial boot or *while the watchdog is watching* — but a kernel can also disappoint a human later, or on a host where rollback is manual by construction (`manual`, `external`). `cmd_rollback` served that case from the start, and for a long time *only* as a CLI flag: the one action a user wants when their kernel misbehaves lived where they would never find it. So `--status` tier [3] now emits **`rollback available: running <X>, known-good <Y>`** whenever `uname -r` differs from `known_good.kver`, and the front-end reveals a **Boot known-good kernel** button on that marker (hidden otherwise, since with nothing to go back to it is noise). The button is a confirmed action that runs `--rollback` unchanged — it re-points the default and uninstalls nothing, which is why §2.5 keeping the previous kernel installed is what makes it work at all.
...
| *any state with no candidate recorded* | watchdog battery fails `trip_after` times | **health warning only**; kernel state untouched | *unchanged* |
