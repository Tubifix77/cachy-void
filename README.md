...
**The gaming-completion set (earlyoom, gamescope, vkBasalt, the Proton toolbox, MangoHud) has been run live**, on the most demanding hardware available for it — a 15-year-old Optimus laptop (`i3-3110M`, legacy `nvidia470`, `GT 730M`). Confirmed on the actual discrete GPU (forced via the same PRIME offload `cachy-game` uses, not the integrated GPU): raw Vulkan/GL rendering works, `cachy-game`'s GameMode composition registers correctly (verified over D-Bus), the MangoHud overlay renders its legacy-Optimus **minimal** profile exactly as designed (no misleading GPU panel), and `earlyoom` runs continuously under runit. That pass also caught and fixed a real bug — vkBasalt silently did nothing without a shipped config file — and pinned down gamescope's known nvidia470 failure to its exact cause (`vkCreateDevice: VkResult -7`, no DRM format-modifier support), a clean failure with no side effects. On capable modern hardware, none of this is expected to be a limiting factor — that's simply the ceiling of the hardware this was tested on.
