...
| **Safe kernel updates** | SHA-256-pinned BORE patch trust, deterministic template regeneration, a config gate that catches silent `oldconfig` drops, and GRUB **one-shot** boot-testing — a bad kernel rolls back on the next power cycle with zero interaction. A pre-flight refuses to start a build that cannot finish, and a failed build pauses the kernel path until a human looks, so the same six-hour failure is never repeated nightly. |
| **Build it on another disk** | A kernel build needs ~20 GB of scratch and won't start below 30 GB free. Rather than reserving a third of a laptop's root partition for something run monthly, point `[build] masterdir` at another disk — an external drive is fine — and the constraint disappears. Validated before it's accepted: space, filesystem, `noexec`, and whether it's USB. |
...
| **The headline** | The pending box leads with the one line that decides whether you press Update — *"12 package updates, 8 driver updates (4 on hold)"*, plus a line naming the **Update kernel** button when a new BORE kernel is available. Counts, not prose; the detail pane below still carries every package and version. |
| **Tray indicator** | `cachy-updater-tray` — a passive presence that notices when updates, a newer BORE kernel to build, a kernel waiting for its first boot, or a failed update run need you, and opens the window on a click. It reads one read-only probe and holds no privilege; it announces a thing once rather than nagging, and re-checks when the updater changes something instead of waiting out its timer. `deploy.sh --no-tray` skips it. |
| **Snapshot visibility** | Every update takes a btrfs snapshot first (§9.5); the **Snapshots** button and `--snapshots` show them — what each update actually did, which ones are pruned automatically, and the exact commands to go back to one *on your layout* (or an honest refusal where that cannot work). Read-only: it never restores anything for you. |
...
Below the buttons a fixed strip carries the two facts that decide how the machine behaves when nobody is watching — whether the nightly run is on, at what time, and whether it includes the kernel; and where the kernel compiles, with the free space there. Both used to live only inside the status report, which scrolls.
...
updater/
  ...
  tests/                 Mock-driven unit + integration suites (667 tests) +
...
**September 2026 — the kernel circuit closed on a relocated build space, and one disk-full event paid for itself.** An unattended kernel build ran out of disk, and because the kernel *failure* path had never been walked before, every latent defect on it surfaced in sequence — each fix reaching one step further than the last. Twenty commits and ~17 bugs later (528 → 667 tests), the picture is materially better than a repair job: the §7.5 pre-flight and the G3 freeze existed in this spec from day one and had never been implemented; `--no-kernel` did not actually remove the kernel from the queue, so *Update* would have compiled a kernel **and skipped the config gate**; the system pass decided from an index `-S` had never refreshed (a dry run performs no sync), so pressing Update installed nothing at all, twice; and a staged candidate was re-detected as a fresh bump on every run, which meant a kernel could never be promoted and the box offered to build what it was already running. Three of those were protected by tests that were green *because they agreed with the bug* — the promote's stored version, a withhold's "no sudo call happened", a flag asserted on the convenient method rather than the important one. New assertions since are mutation-tested: broken deliberately once, to prove they can fail.
